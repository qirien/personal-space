...
label show_credits:
    ...
    show text _("Written, Produced, and Directed by\nAndrea Landaker") with fade
    ...
    show text _("Additional Writing and Design by\nRachel Helps") with fade
    if ((community_level < COMMUNITY_LEVEL_OK) and (ending != "none")):
    ...
    show text _("Chinese localization by:\n\nAlex You")
    show seastar at left, rising behind text
    with dissolve
    $ renpy.pause(4.0, hard=skippable)
    hide seastar
    with dissolve
    ...
    show text _("Made with {a=http://www.renpy.org}Ren'Py{/a}") with fade
    if (ending == "bad"):
